...
post('/new', function (request, response, args) {
	...
	var done   = new Promise(),
	    params = getPostParamsFromRequest(request),
	...
	pres.save(function(err) {
		...
		*/
		/*
		done.resolve(redirect(response, "/" + pres.slug))
	});
...
get('/:slug', function (request, response, args) {
	console.log('anchor')

	var blah = new Promise(),
...
/*
get('/:slug/edit', function (request, response, args) {
...
post('/:slug/edit', function (request, response, args) {
...
function getPostParamsFromRequest (request) {
	...
	return require("querystring").parse(postParams);
}
