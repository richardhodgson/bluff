...
function parse (markdown) {
    ...
    // allow for single newlines before a hr
    while (markdown.match(/(\w)(\n---)/)) {
    ...
    for (var i = 0, l = ast.length; i < l; i++) {
        ...
        switch (ast[i][0]) {
            ...
            case 'hr':
                ...
                break;
            case 'blockquote':
                // markdown-js adds a child parapgraph, remove it
                slideAst[currentSlide].push(
                    ['blockquote', ast[i][1][1]]
                );
                break;
            case 'para':
            case 'numberlist':
